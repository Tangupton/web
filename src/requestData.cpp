...
{
    pthread_once(&once_control, MimeType::init);    //多个线程中,init()函数仅执行一次
...
int RequestData::parse_Headers()
{
    ...
    for (int i = 0; i < str.size() && notFinish; ++i)
    {
        switch(h_state)
        {
            ...
            case h_LF:
            {
                if (str[i] == '\r')         //整个请求头都分析完成了，跳下一步(空行开头)
                {
                    h_state = h_end_CR;
                }
                else                        //没分析完，调到h_key继续从key开始分析(非空行开头)
                { 
                    key_start = i;
                    h_state = h_key;
                }
...
int RequestData::analysisRequest()
{
    if (method == METHOD_POST)
    {
        ...
        header += string("Content-length: ") + to_string(length)+ "\r\n\r\n";
    ...
    else if (method == METHOD_GET)
    {
        ...
        int dot_pos = file_name.find('.');              //分析文件类型
        string filetype;
        if (dot_pos < 0) 
...
void RequestData::handleError(int fd, int err_num, string short_msg)
{
    ...
    body_buff += "<hr><em> noTang's Web Server</em>\n</body></html>";
